...
class APIKeyManager:
    ...
    # ===========================
    # These functions provide backward compatibility for tests and legacy code
    # that still use "team" terminology. They simply delegate to the new "channel" functions.

    @staticmethod
    def create_team(*args, **kwargs):
        """Backward compatibility alias for create_channel_with_key()"""
        return APIKeyManager.create_channel_with_key(*args, **kwargs)

    @staticmethod
    def create_team_with_key(*args, **kwargs):
        """Backward compatibility alias for create_channel_with_key()"""
        return APIKeyManager.create_channel_with_key(*args, **kwargs)

    @staticmethod
    def get_team_by_id(*args, **kwargs):
        """Backward compatibility alias for get_channel_by_id()"""
        return APIKeyManager.get_channel_by_id(*args, **kwargs)

    @staticmethod
    def get_team_by_name(*args, **kwargs):
        """Backward compatibility alias for get_channel_by_title()"""
        return APIKeyManager.get_channel_by_title(*args, **kwargs)

    @staticmethod
    def get_team_by_platform_name(*args, **kwargs):
        """Backward compatibility alias for get_channel_by_channel_id()"""
        return APIKeyManager.get_channel_by_channel_id(*args, **kwargs)

    @staticmethod
    def list_all_teams(*args, **kwargs):
        """Backward compatibility alias for list_all_channels()"""
        return APIKeyManager.list_all_channels(*args, **kwargs)

    @staticmethod
    def list_team_api_keys(*args, **kwargs):
        """Backward compatibility alias for list_channel_api_keys()"""
        return APIKeyManager.list_channel_api_keys(*args, **kwargs)

    @staticmethod
    def update_team(*args, **kwargs):
        """Backward compatibility alias for update_channel()"""
        return APIKeyManager.update_channel(*args, **kwargs)

    @staticmethod
    def delete_team(*args, **kwargs):
        """Backward compatibility alias for delete_channel()"""
        return APIKeyManager.delete_channel(*args, **kwargs)
